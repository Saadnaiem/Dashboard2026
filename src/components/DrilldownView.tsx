...
const DrilldownView: React.FC<DrilldownViewProps> = ({ allRawData, globalFilterOptions, globalData }) => {
    ...
    const {
        ...
        if (searchTerm) finalData = finalData.filter(item =>
            (item.name && String(item.name).toLowerCase().includes(String(searchTerm).toLowerCase())) ||
            (item.code && String(item.code).toLowerCase().includes(String(searchTerm).toLowerCase()))
        );
